...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c => c.toLower).mapValues(g => g.length()).toList.sortBy(p => p._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case Nil => Nil
    case head :: tail => wordOccurrences(s.reduceLeft((a, b) => a + b)).sortBy(p => p._1)
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary
      ...
      .groupBy(w => w._1)
      .mapValues(words => words.map(x => x._2))

  /** Returns all the anagrams of a given word. */
  def wordAnagrams(word: Word): List[Word] =
    dictionaryByOccurrences.
      find(o => o._1 == wordOccurrences(word)).
      map(x => x._2) getOrElse Nil
